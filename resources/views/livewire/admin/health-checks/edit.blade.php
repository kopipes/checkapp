<div class="max-w-2xl mx-auto">
    <div class="mb-6">
        ...
        <h2 class="page-title mt-1">Edit Pemeriksaan</h2>
        <p class="page-subtitle">{{ $healthCheck->user->name }} &mdash; {{ $healthCheck->check_date->format('d M Y') }}</p>
    </div>
    ...
    <form wire:submit="save" class="space-y-4">
        ...
        <div class="card p-5">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                ...
                @foreach ($fields as $f)
                    <div>
                        <label class="label">{{ $f['label'] }} <span class="text-gray-400 font-normal text-xs ml-1">{{ $f['unit'] }}</span></label>
                        <input wire:model="{{ $f['model'] }}" type="number"
                ...
                <div class="sm:col-span-2">
                    ...
                    <textarea wire:model="notes" rows="3" class="input resize-none"></textarea>
                </div>
            </div>
        </div>
